...
- [x] **Phase 8c (~2d)**: `backtest/runner.py` (BacktestScorer Protocol + V8LiteScorer + walk-forward harness + PG-backed input loader) + `calibration.py` (constrained grid search + monotonicity + version_manager hook) + `reports.py` (markdown with metrics/baseline/case study/calibration sections) + `scripts/run_backtest.py` CLI + 32 新单测 (runner 11 + calibration 13 + reports 8) + 2 integration smoke tests. Bayesian 优化退回 constrained grid search（ADR 0013 §Negative 允许，避免新依赖）。
- [x] **Phase 8d (~0.5d)**: `api/routers/backtest.py` 实装（list runs + detail by run_id + _meta/count；6 单测）+ `runner.persist_run_to_pg` (复用 prediction_snapshots.config_snapshot.backtest_run_id，不开新表) + CLI `--persist` flag + **374 样本全量回测跑通**（374 IPO 评估 / 277 regime-pass / 持久化 374 prediction_snapshots / 报告写入 `reports/backtest/2026-05-17_*.md`）+ ETL bug fix (scripts/migrate_sqlite_to_pg.py: SQLite link 表无 as_of_date 列，disclosure_date 留 null) + tag `v0.8`
