...
- [x] **Phase 2**：`scripts/migrate_sqlite_to_pg.py` 实现 §1 全部映射并通过幂等测试（实测 399 IPO + 2014 基石 + 2560 link + 1592 财报，9 integration tests pass）
- [x] **Phase 2**：`data/builders/historical_ipo_loader.py` 优先从迁移后的 PG 表加载，缺字段才回退 iFind（iFind 增量加载留待 Phase 2.1 凭证就绪后）
