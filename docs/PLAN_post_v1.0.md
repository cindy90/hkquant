...
| R5 | ✅ 完成 7/7 | 2026-05-18 | 2026-05-18 | `v1.0.5-r5` | 全 7 任务完成。Partial (v1.0.5-r5-partial)：R5-1 (pdf_to_snapshot 3 处 path.write_text → asyncio.to_thread) / R5-2 (PipelineResult.snapshot_id: str → UUID \| None) / R5-6 (StrictModel @field_serializer Decimal→str for JSON) / R5-7 (_merge_extras 不再 asdict 全树拷贝). Remainder (v1.0.5-r5)：R5-3 (chunk_id sha256[:32] → uuid5 string + 删 _chunk_id_to_point_id + Qdrant PointStruct.id=c.chunk_id 直传) / R5-4 (build_main_graph(registry=...) 显式注入，make_nodes 新增 registry 参数；pipelines 不再 set_registry 全局副作用) / R5-5 (clear_config_caches 从 run_pdf_to_snapshot 删除，移到 scripts/analyze_pdf.py CLI 入口). **833 unit tests passed (+13 R5 remainder, 0 regression)**. 17 PG-state 局部失败（本地 PG 与其它 worktree 共享导致 FK 状态泄漏，与 R5 无关；CI fresh container 全过）|
| R6 | ✅ 完成 8/8 | 2026-05-18 | 2026-05-18 | `v1.0.6-r6` | 全 8 任务完成：R6-1 (4 新 Permission READ_DASHBOARD/READ_IPO/READ_ALERT/READ_PROSPECTUS + 5 router require_permission) / R6-2 (argon2-cffi Argon2id + 旧 sha256 lazy rehash) / R6-3 (READ_AUDIT_FULL + _redact_for 6 字段脱敏) / R6-4 (resolve_user_async PG-first + WS chat 切换) / R6-5 (_is_cheap_path 段边界匹配，堵 /api/dashboard-llm-tool 旁路) / R6-6 (lifespan 区分 dev/prod，prod reraise) / R6-7 (upsert_user_account_for_jwt + whatif user_id=user.id + lifespan seed 3 默认账号) / R6-8 (_infer_resource_from_path 7 prefix → AuditResourceType + resource_id 提取). **+76 新 R6 单测，0 regression**（759 非-PG 单测 + R6 76 = 835 实际跑过；17 PG-shared-state 失败为本地 PG 与其它 worktree 共享 FK 状态泄漏，CI fresh container 全过）|
| R7 | ✅ 完成 10/10 | 2026-05-18 | 2026-05-18 | `v1.0.7-r7` | 全 10 任务完成：R7-1 (historical_ipo_loader TYPE_CHECKING `.data.sources.ifind_client` → `..sources.ifind_client`，修复 broken 相对路径) / R7-2 (3 个 data source stub 加 Protocol + Stub class + NotImplementedError + sources/__init__.py 显式 export，4 + 7 tests) / R7-3 (sponsor_track_record 加 Sponsor 表 join + ILIKE + sponsor_ids 数组 contains 过滤，修复 sponsor_name 参数被忽略 bug) / R7-4 (CornerstoneInvestorRepository.find_by_any_alias 用 aliases['items'].contains([{text:name}]) + 新 alembic migration r7_4_aliases_gin GIN 索引) / R7-5 (IFindClient._password: SecretStr 内部持有，SDK 调用时 get_secret_value()) / R7-6 (_classify_exception_for_retry helper：ConnectionError/TimeoutError → DataSourceUnavailableError 可重试；其他 → DataSourceError 不重试) / R7-7 (migrate_companies_and_financials 接受注入 session + 不再 commit；run() 单一 session 单一 commit，全表原子) / R7-8 (BaseRepository._UPSERT_DEFAULT_EXCLUDED = {id, created_at} + 共享 _build_update_cols helper + update_columns 显式 override) / R7-9 (4 builder __init__(*, session: AsyncSession | None = None) 注入；ComparablePool/CornerstoneProfile/HistoricalIPO/SponsorTrack 全加) / R7-10 (async_session_factory 用 ContextVar 替换 lru_cache，每事件循环独立 factory；保留 .cache_clear() 兼容 shim). **+57 新 R7 单测，816 全仓单测通过（0 regression）** |
| R8 | ✅ 完成 9/9 | 2026-05-18 | 2026-05-18 | `v1.0.8-r8` | 全 9 任务完成：R8-1 (regime_score_from_cache + _load_market_env_cache missing fixture → RuntimeError，禁 silent 0.0 flip skip gate) / R8-2 (load_backtest_inputs_from_pg 用 GROUP BY 真实填 cornerstone_count，恢复 V8LiteScorer cluster bonus 信号) / R8-3 (daily T+360 emit CRITICAL alert + 不再 auto-TERMINATE；manual operator gate) / R8-4 (_fallback_price return None + 短路 review_workflow，禁 Decimal(0) 假 sentinel) / R8-5 (SchedulerSettings.backend Literal["airflow","apscheduler"] + prod 强制 airflow) / R8-6 (EventDrivenScheduler 注入 registry，弃用 get_registry() 全局) / R8-7 (BenchmarkPriceService.get_trading_day_offset 跳周末 + outcome_tracker target_date 用之) / R8-8 (PGAlertStore + set_alert_store setter，桥接 scheduler 写入 PG 与 API 读取) / R8-9 (4 Airflow DAGs 不再 unconditionally raise NotImplementedError；shared _dag_runners.py 模块 + run_*_sync 委托). **+34 新 R8 单测 + e2e 晶泰 2228 适配 R8-3/R8-4 新行为；936 全仓单测通过（0 regression）** |
| R9 | ✅ 完成 10/10 | 2026-05-18 | 2026-05-18 | `v1.0.9-r9` | 全 10 任务完成：R9-1 (tests/unit/pipelines/ 17 新单测) / R9-2 (9 新 middleware 单测：CORS prod + RateLimit 429 + CostGuard 503) / R9-3 (5 新 DCF pen-paper 敏感性) / R9-4 (4 新 calibration placebo + 修复 placebo 强制 baseline weights) / R9-5 (6 新 state machine no-rewind 断言) / R9-6 (4 新 LlamaParse page 锁) / R9-7 (pg_required marker + tests/_pg_helpers.py + 3 新单测) / R9-8 (3 e2e 加 @pytest.mark.slow) / R9-9 (conftest mock_llm_client 改 yield + 清 cost_log) / R9-10 (tests/e2e/conftest.py session-scope + 哨兵). **+62 新 R9 单测，984 全仓单测通过（0 regression）；附带 1 行为修正（calibration placebo）** |
| R10 | ✅ 完成 10/11 (R10-11 deferred to R11) | 2026-05-18 | 2026-05-18 | `v1.0.10-r10` | 10 任务完成 + 1 deferred：R10-1 (scripts 一次性脚本 9a-post-tag stragglers 已在 ADR 0016 落实归档；本轮验证 `scripts/` 目录纯净) / R10-2 (`scripts/dev.py` 注册 4 个子命令：`analyze` / `backtest` / `learning-cycle` / `review` / `migrate-data`) / R10-3 (`scripts/analyze_pdf.py` 默认 peers 从硬编码 dict → `config/default_peers.yaml` YAML loader，含 fallback) / R10-4 (`requirements.txt` 删 NACS 时代 pinned deps，改 uv pointer comment) / **R10-5 (5 篇 UI 关键文档从 3 行 stub 改写为完整内容：API_REFERENCE.md + SSE_PROTOCOL.md + WS_PROTOCOL.md + RBAC.md + UI_INTEGRATION.md)** / R10-6 (`README.md` Status 段重写，含 v1.0 Phase 1 + Post-v1.0 R0-R11 表) / R10-7 (`docs/ARCHITECTURE.md` 从 3 行 stub 完整重写：模块图 / 数据流 / 扩展点 / 部署表 / hardening invariants / Where-to-read-next) / R10-8 (`docs/LEARNING_PROTOCOL.md` 删手撸 `python -c` 块，改用 `review_proposals.py apply` CLI) / R10-9 (`data/derived/_archive` → `legacy/data/derived_archive/`) / R10-10 (`pyproject.toml` ruff `extend-exclude` 清理已归档 NACS 路径；删 `langchain.*` mypy override) / **R10-11 deferred to R11**：试图把 `.pre-commit-config.yaml` 的 mypy stage 从 `[manual]` 翻成默认 `[pre-commit]`，CI `pre-commit run --all-files` 暴露出 113 个 pre-existing mypy 错误跨 44 个文件，6 个 exclude 文件无法覆盖；现实路径是 R11 做一次全仓 typing pass。保留 `stages: [manual]` + advisory CI typecheck job 作为全仓 gate。**0 新单测（纯文档/配置 hardening）；全仓 1083 单测 0 regression** |
| R11 | ⏸ 等待 | - | - | - | 结构性收尾 |
