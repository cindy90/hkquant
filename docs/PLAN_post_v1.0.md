...
| R5 | 🔄 4/7 | 2026-05-18 | - | `v1.0.5-r5-partial` | R5-1 (pdf_to_snapshot 3 处 path.write_text → asyncio.to_thread) / R5-2 (PipelineResult.snapshot_id: str → UUID \| None) / R5-6 (StrictModel @field_serializer Decimal→str for JSON) / R5-7 (_merge_extras 不再 asdict 全树拷贝). **820 unit tests passed (+10)**. R5-3/4/5 (chunk_id UUID / set_registry 副作用 / contextvar) 待做 |
| R6 | ⏸ 等待 | - | - | - | RBAC + Auth |
| R7 | ⏸ 等待 | - | - | - | 数据层 |
| R8 | ⏸ 等待 | - | - | - | 调度器 + 警报 |
| R9 | ⏸ 等待 | - | - | - | 测试缺口 |
