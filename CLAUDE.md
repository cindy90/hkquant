...
| **Phase 5** Agent 层 | **ADR 0005 §2 + §5**（强制） + **ADR 0009**（港股研究agent 框架借鉴） + **ADR 0019**（post-P1 frontmatter schema） + PROJECT_SPEC.md §7 | `policy_agent` 必须输出 `regime_score`；`cornerstone_signal_agent` 必须用 ultimate_holder 聚类；`sentiment_agent` 必须读 `themes/*.json`；3 个 prompts 的 `requires_extras` frontmatter 由 `BaseAgent._assert_required_extras` 在 LLM 调用前硬断言（缺则 raise `MissingInheritedInput`）；公共片段 `prompts/system/agent_common.md` 通过 Jinja2 include 共享；BaseAgent / ScoreCard / WorkflowExtras 参考港股研究agent (ADR 0009) |
