...
async def test_daily_scheduler_tracks_due_checkpoints_idempotently(fresh_sf) -> None:
    ...
    from hk_ipo_agent.common.llm_client import LLMClient  # noqa: PLC0415
    from hk_ipo_agent.prediction_registry.attribution import (  # noqa: PLC0415
        AttributionEngine,
        _DiagnosisOutput,
    )
    ...
    llm.acomplete_json = AsyncMock(  # type: ignore[method-assign]
        return_value=_DiagnosisOutput(
            primary_attribution="within_tolerance", llm_diagnosis="ok",
    ...
    repo = _StubIPORepo({
        snap.ipo_id: IPOMetadata(
            ipo_id=snap.ipo_id, stock_code="TEST.HK",
            listing_date=listing_d, industry_peers=[], actual_price_at_checkpoint={},
        ),
    })
    sched = DailyScheduler(
        session_factory=fresh_sf, state_machine=sm,
...
async def test_daily_scheduler_back_fills_missed_checkpoints(fresh_sf) -> None:
    ...
    repo = _StubIPORepo({
        snap.ipo_id: IPOMetadata(
            ipo_id=snap.ipo_id, stock_code="TEST.HK", listing_date=listing_d,
            industry_peers=[], actual_price_at_checkpoint={},
        ),
    })
    sched = DailyScheduler(
        session_factory=fresh_sf, state_machine=sm,
...
async def test_daily_scheduler_terminates_at_360_days(fresh_sf) -> None:
    ...
    registry = PGPredictionRegistry(session_factory=fresh_sf)
    tracker = OutcomeTracker(
        session_factory=fresh_sf, snapshot_resolver=registry,
        benchmarks=_StubBenchmarks(), price_fetcher=_StubPrices(),
    )
    from hk_ipo_agent.common.llm_client import LLMClient  # noqa: PLC0415
    from hk_ipo_agent.prediction_registry.attribution import AttributionEngine  # noqa: PLC0415

    llm = LLMClient(api_key="sk-test", daily_budget_usd=Decimal("100"))
    llm.acomplete_json = AsyncMock()  # type: ignore[method-assign]
    workflow = ReviewWorkflow(
        registry=registry, attribution=AttributionEngine(llm=llm), session_factory=fresh_sf,
    )
    repo = _StubIPORepo({
        snap.ipo_id: IPOMetadata(
            ipo_id=snap.ipo_id, stock_code="TEST.HK", listing_date=listing_d,
...
async def test_high_freq_advances_pricing_state_on_signal(fresh_sf) -> None:
    ...
    from hk_ipo_agent.prediction_registry.ipo_lifecycle.state_detectors import (  # noqa: PLC0415
        StateDetectors,
        TransitionSignal,
    )

    detectors = MagicMock(spec=StateDetectors)
    detectors.detect_pricing = AsyncMock(return_value=TransitionSignal(
    ...
    repo = MagicMock()
    repo.get_context = AsyncMock(return_value=__import__(
    ...
    sched = HighFrequencyScheduler(
        session_factory=fresh_sf, state_machine=sm,
        state_detectors=detectors, ipo_repo=repo,
    )
    result = await sched.run()
    assert result.status is SchedulerStatus.COMPLETED
...
async def test_event_driven_routes_earnings_to_comparator(fresh_sf) -> None:
    ...
    queue.pull = AsyncMock(return_value=[
        EventPayload(
            kind=EVENT_KIND_EARNINGS, ipo_id=snap.ipo_id,
    ...
    from hk_ipo_agent.prediction_registry.registry import (  # noqa: PLC0415
    ...
    sched = EventDrivenScheduler(
        session_factory=fresh_sf, queue=queue,
        earnings_comparator=earnings_comparator, state_machine=sm,
...
async def test_event_driven_routes_critical_price_anomaly_to_alerts(fresh_sf) -> None:
    ...
    queue.pull = AsyncMock(return_value=[
        EventPayload(
            kind=EVENT_KIND_PRICE_ANOMALY, ipo_id=ipo_id,
            occurred_at=datetime.now(UTC),
    ...
    sched = EventDrivenScheduler(
        session_factory=fresh_sf, queue=queue,
        earnings_comparator=earnings, state_machine=sm,
        snapshot_resolver=snapshot_resolver, alert_router=alerts,
    )
    await sched.run()
...
async def test_event_driven_ignores_unknown_event_kinds(fresh_sf) -> None:
    ...
    queue.pull = AsyncMock(return_value=[
        EventPayload(
            kind="something_unsupported", ipo_id=uuid.uuid4(),
            occurred_at=datetime.now(UTC), payload={},
        ),
    ])
    sched = EventDrivenScheduler(
        session_factory=fresh_sf, queue=queue,
        earnings_comparator=MagicMock(), state_machine=StateMachine(fresh_sf),
        snapshot_resolver=MagicMock(),
    )
    result = await sched.run()
